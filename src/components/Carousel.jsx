  ...
  return (
    ...
    <div className="relative w-full h-full overflow-hidden">
      <Transition show={show}
        ...
        <img src={images[index]} alt="imagen"className="w-full h-auto object-cover"/>

      </div>
      </Transition>
      <button onClick={previousImage} className="absolute left-0 top-1/2 -translate-y-1/2 bg-sky-600 text-white font-bold p-4">
        {"<"}      
      </button>
      <button onClick={nextImage} className="absolute right-0 top-1/2 -translate-y-1/2 bg-sky-600 text-white font-bold p-4">
        {">"}      
      </button>
    </div>
  )
